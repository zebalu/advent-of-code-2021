...
public class Day04 {
    public static void main(String[] args) {
        ...
        int[] draw = readDrawn(lines.get(0));
        firstPart(draw, readBoards(lines));
        secondPart(draw, readBoards(lines));
    }

    private static List<Board> readBoards(List<String> lines) {
        List<Board> boards = new ArrayList<>();
        for (int i = 2; i < lines.size(); i += 6) {
            List<String> collect = lines.subList(i, i + 5);
            boards.add(new Board(collect));
        }
        return boards;
    }

    private static int[] readDrawn(String drawLine) {
        int[] draw = Arrays.stream(drawLine.split(",")).mapToInt(Integer::parseInt).toArray();
        return draw;
    }
    ...
    private static class Board {
        ...
        Board(List<String> input) {
            for (int x = 0; x < input.size(); ++x) {
                var m = NUM_PATTERN.matcher(input.get(x));
                if (!m.matches()) {
                    throw new IllegalArgumentException("Can not understand: " + input.get(x));
                }
                for (int y = 0; y < 5; ++y) {
                    boardNums.put(new Coord(x, y), Integer.parseInt(m.group(y + 1)));
                }
            }
        }

        boolean draw(int num) {
            drawn.add(num);
            return isWinning();
        }
        ...
        private int get(int x, int y) {
            return boardNums.get(new Coord(x, y));
        }
    }
