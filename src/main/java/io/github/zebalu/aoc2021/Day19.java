...
public class Day19 {
    ...
    private static void secondPart(List<Scanner> scanners) {
        ...
        for (var s1 : scanners) {
            for (var s2 : scanners) {
                var distance = s1.center.distance(s2.center);
    ...
    private static final class Scanner {
                ...
                for (var turn : Coord.turns) {
                    ...
                    for (int tt =0; tt<transformedCoords.size()-12; ++tt) {
                        ...
                        var count = transformedCoords.stream().map(t -> t.minus(diff)).filter(myFixCoords::contains)
                                .count();
                        if (count >= 12) {
                            var diff2 = diff.multiply(new Coord(-1, -1, -1));
                            other.center = diff2;
                            return turn.andThen(c -> c.minus(diff));
                        }
                    }
                }
            }
        ...
        void transform(Function<Coord, Coord> transform) {
            coords = coords.stream().map(transform).toList();
        }

        static Scanner read(String scannerScript) {
            var lines = scannerScript.lines().toList();
            List<Coord> coords = new ArrayList<>();
            for (int i = 1; i < lines.size(); ++i) {
                var nums = lines.get(i).split(",");
                coords.add(new Coord(Integer.parseInt(nums[0]), Integer.parseInt(nums[1]), Integer.parseInt(nums[2])));
            }
            return new Scanner(coords);
        }
    }

    private static final record Coord(int x, int y, int z) {
