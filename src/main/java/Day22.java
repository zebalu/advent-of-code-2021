...
public class Day22 {
    ...
    private static void firstPart() {
        var commands = INPUT.lines().map(SwitchCommand::fromString).toList();
        Reactor reactor = new Reactor(-50, 50, -50, 50, -50, 50);
        commands.forEach(reactor::execute);
        System.out.println(reactor.countOn());
    }
    
    private static void secondPart() {
        var initReactors = INPUT.lines().map(SwitchCommand::fromString).map(SummarizingReactor::new).toList();
        List<SummarizingReactor> reactors = new LinkedList<>();
        for (var reactor : initReactors) {
            var distinctReactors = reactors.stream().map(r -> r.substract(reactor)).flatMap(List::stream).toList();
            reactors.clear();
            reactors.addAll(distinctReactors);
            if (reactor.startsWithOn) {
                reactors.add(reactor);
            }
        }
        System.out.println(reactors.stream().mapToLong(SummarizingReactor::countOn).sum());
    }

    private static record Coord(long x, long y, long z) {
        static Coord fromStrings(String x, String y, String z) {
            return new Coord(Integer.parseInt(x), Integer.parseInt(y), Integer.parseInt(z));
        }
    }

    private static record SwitchCommand(Coord min, Coord max, boolean on) {

        private static final Pattern SWITCH_PATTERN = Pattern
                .compile("(on|off) x=(-?\\d+)..(-?\\d+),y=(-?\\d+)..(-?\\d+),z=(-?\\d+)..(-?\\d+)");
        static SwitchCommand fromString(String line) {
            var m = SWITCH_PATTERN.matcher(line);
            if (!m.matches()) {
                throw new IllegalArgumentException("can't read line: '" + line + "'");
            }
            boolean on = m.group(1).equals("on");
            Coord min = Coord.fromStrings(m.group(2), m.group(4), m.group(6));
            Coord max = Coord.fromStrings(m.group(3), m.group(5), m.group(7));
            return new SwitchCommand(min, max, on);
        }
    }

    private static class Reactor implements Cloneable {
        protected final long minX;
        protected final long maxX;
        protected final long minY;
        protected final long maxY;
        protected final long minZ;
        protected final long maxZ;
        protected Map<Coord, Boolean> cores = new HashMap<>();

        public Reactor(long minX, long maxX, long minY, long maxY, long minZ, long maxZ) {
            super();
            this.minX = minX;
            this.maxX = maxX;
            this.minY = minY;
        ...
        void execute(SwitchCommand command) {
            for (long x = Math.max(minX, command.min.x); x <= Math.min(maxX, command.max.x); ++x) {
                for (long y = Math.max(minY, command.min.y); y <= Math.min(maxY, command.max.y); ++y) {
                    for (long z = Math.max(minZ, command.min.z); z <= Math.min(maxZ, command.max.z); ++z) {
                        if (command.on) {
                            cores.put(new Coord(x, y, z), true);
                        } else {
                            cores.put(new Coord(x, y, z), false);
                        }
                    }
                }
            }
        ...
        long countOn() {
            return count(Boolean::booleanValue);
        }
        ...
        private long count(Predicate<Boolean> filter) {
            return cores.values().stream().filter(filter).count();
        }

        @Override
        public Reactor clone() {
            try {
                Reactor clone = (Reactor) super.clone();
                clone.cores = new HashMap<>();
                clone.cores.putAll(cores);
                return clone;
    ...
    private static class SummarizingReactor extends Reactor implements Cloneable {
        private final boolean startsWithOn;

        public SummarizingReactor(SwitchCommand initCommand) {
            super(initCommand.min.x, initCommand.max.x, initCommand.min.y, initCommand.max.y, initCommand.min.z,
                    initCommand.max.z);
            startsWithOn = initCommand.on;
        }

        @Override
        long countOn() {
            if (startsWithOn) {
                return (maxX - minX + 1) * (maxY - minY + 1) * (maxZ - minZ + 1) - countOff();
            } else {
                return super.countOn();
            }
        }
        ...
        List<SummarizingReactor> substract(SummarizingReactor other) {
            ...
            List<Long> xCuts = cutPoints(minX, maxX, other.minX, other.maxX);
            List<Long> yCuts = cutPoints(minY, maxY, other.minY, other.maxY);
            List<Long> zCuts = cutPoints(minZ, maxZ, other.minZ, other.maxZ);
            List<SummarizingReactor> reactores = new LinkedList<>();
            for (int x = 0; x < xCuts.size() - 1; ++x) {
                for (int y = 0; y < yCuts.size() - 1; ++y) {
                    for (int z = 0; z < zCuts.size() - 1; ++z) {
                        reactores.add(new SummarizingReactor(
                                new SwitchCommand(new Coord(xCuts.get(x), yCuts.get(y), zCuts.get(z)),
                                        new Coord(xCuts.get(x + 1) - 1, yCuts.get(y + 1) - 1, zCuts.get(z + 1) - 1),
                                        startsWithOn)));
                    }
                }
            }
            return reactores.stream().filter(r -> !other.contains(r)).filter(SummarizingReactor::isValid).toList();
        }

        boolean contains(SummarizingReactor that) {
            return minX <= that.minX && that.maxX <= maxX && minY <= that.minY && that.maxY <= maxY && minZ <= that.minZ
                    && that.maxZ <= maxZ;
        }

        boolean hasIntersection(SummarizingReactor other) {
            return minX <= other.maxX && other.minX <= maxX && minY <= other.maxY && other.minY <= maxY
                    && minZ <= other.maxZ && other.minZ <= maxZ;
        }

        List<Long> cutPoints(long min, long max, long otherMin, long otherMax) {
            List<Long> result = new LinkedList<>();
            result.add(min);
            if (min < otherMin && otherMin <= max) {
                result.add(otherMin);
            }
            if (min <= otherMax && otherMax <= max - 1) {
                result.add(otherMax + 1);
            }
            result.add(max + 1);
            return result;
        }
        ...
        @Override
        public SummarizingReactor clone() {
            return (SummarizingReactor) super.clone();
        }
    }
